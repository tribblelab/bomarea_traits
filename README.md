...
3. In bash, combine and remove the first 10% of data:
   ...
   - Run the code to combine the runs (`.txt` files):
     ```bash
     awk 'FNR == 1 && NR != 1 { next } { print }' branchiness_2rates_rj_ard_states_run_1.txt branchiness_2rates_rj_ard_states_run_2.txt > branchiness_2rates_rj_ard_states_combined.txt
     ```
